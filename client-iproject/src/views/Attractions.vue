...
export default {
  ...
  computed: {
    ...mapState(useStoreIndex, ["attractions", "currentPage", 'covid19']),
  ...
  methods: {
    ...mapActions(useStoreIndex, ["listAttractions"]),
  ...
  created() {
    this.listAttractions(1);
  },
  components: { Carousel, CardAttraction },
};
</script>
...
<template>
  ...
  <section id="home">
    ...
    <div class="container mb-4" style="margin-top: 3em">
      <div class="row">
        <CardAttraction v-for="(attraction, index) in attractions" :key="index" :attraction="attraction" />
      </div>
    </div>
